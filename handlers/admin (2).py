...
from database import get_conn, get_all_users, get_setting, set_setting, delete_setting, get_sections, add_section, delete_section
...
def _admin_kb():
    ...
    api_status = "🟢 AI API Set" if api_key else "🔴 AI API Missing"
    return Markup([
        ...
        [Btn(f"🤖 {api_status}",   callback_data="admin_ai_api")],
...
# ════════════════════════════════════════════════════════════════════════════
async def admin_ai_api(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    api_key = get_setting("gemini_api_key")

    if api_key:
        # Show masked key and options
        masked = api_key[:8] + "•" * (len(api_key) - 12) + api_key[-4:]
        text   = (
            f"🤖 *Gemini AI API*\n\n"
            f"Status: 🟢 *Active*\n"
            f"Key: `{masked}`\n\n"
            f"AI Doubt Solver is currently *working*."
        )
        kb = Markup([
            [Btn("🔄 Update API Key", callback_data="admin_ai_set"),
             Btn("🗑️ Delete API Key", callback_data="admin_ai_delete")],
            [Btn("🧪 Test API",       callback_data="admin_ai_test")],
            [Btn(f"{E['back']} Back", callback_data="admin_panel_back")],
        ])
    else:
        text = (
            f"🤖 *Gemini AI API*\n\n"
            f"Status: 🔴 *Not Set*\n\n"
            f"Gemini API key set karo taaki students AI se doubts pooch sakein.\n\n"
            f"👉 API key yahan se lo: [Google AI Studio](https://aistudio.google.com/apikey)"
        )
        kb = Markup([
            [Btn("➕ Set API Key",    callback_data="admin_ai_set")],
            [Btn(f"{E['back']} Back", callback_data="admin_panel_back")],
        ])
    await query.edit_message_text(text, reply_markup=kb, parse_mode="Markdown",
...
async def admin_ai_set_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text(
        "🔑 *Set Gemini API Key*\n\n"
        "Apni Gemini API key bhejo:\n"
        "_(Google AI Studio → Get API Key)_\n\n"
        "⚠️ Key is message mein bhejo — baad mein bot delete kar dega.",
        reply_markup=cancel_btn("admin_ai_api"), parse_mode="Markdown"
    )
    return GEMINI_API_INPUT


async def admin_ai_got_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(key) < 20:
        await update.message.reply_text(
            "❌ Invalid API key (too short). Please send a valid Gemini API key.",
            reply_markup=cancel_btn("admin_ai_api")
        )
        return GEMINI_API_INPUT
    ...
    valid = await _test_gemini_key(key)
    ...
    if valid:
        ...
        await context.bot.send_message(
            ...
            "🟢 AI Doubt Solver is now *active*.\n"
            "Students ab AI se doubts pooch sakte hain.",
            reply_markup=back_btn("admin_ai_api"),
            parse_mode="Markdown"
        )
    else:
        await context.bot.send_message(
            update.effective_user.id,
            "❌ *Invalid API key!*\n\n"
            "Key save nahi ki gayi. Please correct key bhejo.",
            reply_markup=cancel_btn("admin_ai_api"),
            parse_mode="Markdown"
        )
        return GEMINI_API_INPUT
...
async def admin_ai_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    from ui import confirm_delete_kb
    await query.edit_message_text(
        "⚠️ *Delete API Key?*\n\nAI Doubt Solver band ho jaayega.",
        reply_markup=confirm_delete_kb("admin_ai_del_yes", "admin_ai_api"),
        parse_mode="Markdown"
    )
    return ConversationHandler.END


async def admin_ai_del_yes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    delete_setting("gemini_api_key")
    await query.edit_message_text(
        "🗑️ API key deleted.\n🔴 AI Doubt Solver is now *inactive*.",
        reply_markup=back_btn("admin_ai_api"), parse_mode="Markdown"
    )
    return ConversationHandler.END


async def admin_ai_test(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer("Testing...")
    api_key = get_setting("gemini_api_key")
    if not api_key:
        await query.answer("No API key set!", show_alert=True)
        return ConversationHandler.END
    valid = await _test_gemini_key(api_key)
    if valid:
        await query.answer("✅ API key is working!", show_alert=True)
    else:
        await query.answer("❌ API key is NOT working! Please update it.", show_alert=True)
    return ConversationHandler.END


async def _test_gemini_key(api_key: str) -> bool:
    """Test if Gemini API key is valid. Returns True if valid."""
    try:
...
def build_admin_conv():
    return ConversationHandler(
        entry_points=[
            ...
            # AI API Management
            CallbackQueryHandler(admin_ai_api,       pattern="^admin_ai_api$"),
            CallbackQueryHandler(admin_ai_set_start, pattern="^admin_ai_set$"),
            CallbackQueryHandler(admin_ai_delete,    pattern="^admin_ai_delete$"),
            CallbackQueryHandler(admin_ai_del_yes,   pattern="^admin_ai_del_yes$"),
            CallbackQueryHandler(admin_ai_test,      pattern="^admin_ai_test$"),
        ],
        states={
            ...
            GEMINI_API_INPUT:   [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_ai_got_key)],
        },
        fallbacks=[
            CallbackQueryHandler(admin_panel_cb, pattern="^admin_panel_back$"),
