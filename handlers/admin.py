...
async def abk_class(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data["book_draft"] = {"class_num": query.data.split("_")[2]}
...
async def abk_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.execute(
        ...
        (d.get("class_num"), d.get("subject"), d.get("book_name"), file_id, update.effective_user.id)
    )
    conn.commit(); conn.close()
...
async def apyq_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_conn()
    conn.execute(
        "INSERT INTO pyqs (exam_type, title, file_id, file_name, added_by) VALUES (?,?,?,?,?)",
        (d.get("exam_type"), d.get("title"), file_id, file_name, update.effective_user.id)
    )
    conn.commit(); conn.close()
    count = d.get("count", 0) + 1
...
async def amix_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_conn()
    conn.execute(
        "INSERT INTO mix_books (book_name, file_id, file_name, added_by) VALUES (?,?,?,?)",
        (d.get("book_name"), file_id, file_name, update.effective_user.id)
    )
    conn.commit(); conn.close()
    count = d.get("count", 0) + 1
