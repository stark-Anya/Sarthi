...
(
    ...
    BROADCAST_TEXT,
    DB_CLEAR_PASS_STATE,
    BOOK_CLASS, BOOK_SUBJ, BOOK_NAME, BOOK_FILE,
) = range(11)
...
async def cleardb_got_pass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cleardb_type == "user":
        # Delete all user data EXCEPT formulas, books and user accounts
        ...
        await update.message.reply_text(
            ...
            "revisions, study logs deleted.\nFormulas, books and user accounts are intact.",
        ...
        for table in ["tasks", "memories", "daily_reports", "thoughts", "motivation",
                      "study_log", "test_scores", "revision_schedule", "doubts", "lectures",
                      "formulas", "books", "users"]:
            conn.execute(f"DELETE FROM {table}")
        conn.commit()
        conn.close()
        _authed.discard(update.effective_user.id)
        await update.message.reply_text(
            "💥 *Full database cleared!*\n\nAll data including formulas, books and user accounts deleted.\n"
...
async def admin_book_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "📚 *Book Management*\n\nUpload PDFs or delete existing books.",
        reply_markup=kb, parse_mode="Markdown"
    )
    return ConversationHandler.END
...
async def abk_class(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    class_num = query.data.split("_")[2]
    context.user_data["book_draft"] = {"class_num": class_num}
    await query.edit_message_text(
        "Select subject for this book:",
        reply_markup=subject_kb("abks")
    )
    return BOOK_SUBJ
...
async def abk_subj(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "Enter the *book name* (e.g. `HC Verma Vol 1`, `NCERT Chemistry`):",
...
async def abk_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Now send the *PDF file* for:\n\n📖 *{d['book_name']}*\nClass {d['class_num']} | {d['subject']}",
        reply_markup=cancel_btn("admin_book_menu"), parse_mode="Markdown"
    )
    return BOOK_FILE
...
async def abk_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.document:
        await update.message.reply_text(
            "❌ Please send a *PDF file* (document), not a photo.",
            reply_markup=cancel_btn("admin_book_menu"), parse_mode="Markdown"
        )
        return BOOK_FILE
    file_id = update.message.document.file_id
    conn = get_conn()
    conn.execute(
        "INSERT INTO books (class_num, subject, book_name, file_id, added_by) VALUES (?,?,?,?,?)",
...
async def admin_book_del_subj(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        await query.edit_message_text(
            f"No books for Class {class_num}.", reply_markup=back_btn("admin_book_del_class")
        )
        return ConversationHandler.END
    ...
    await query.edit_message_text(
        f"Class {class_num} — Select subject:", reply_markup=Markup(kb_rows)
    )
    return ConversationHandler.END
...
async def admin_book_del_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        await query.edit_message_text(
            "No books found.", reply_markup=back_btn(f"abkdel_class_{class_num}")
        )
        return ConversationHandler.END
    kb_rows = [[Btn(f"🗑️ {r['book_name']}", callback_data=f"abkdel_confirm_{r['id']}")] for r in rows]
    kb_rows.append([Btn(f"{E['back']} Back", callback_data=f"abkdel_class_{class_num}")])
    await query.edit_message_text(
        f"Class {class_num} | {subject} — Select book to delete:",
        reply_markup=Markup(kb_rows)
    )
    return ConversationHandler.END
...
async def admin_book_del_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not book:
        ...
        return ConversationHandler.END
    await query.edit_message_text(
        f"⚠️ Delete book *{book['book_name']}*?\nClass {book['class_num']} | {book['subject']}",
        reply_markup=confirm_delete_kb(
            f"abkdel_yes_{book_id}",
            "admin_book_menu"
        ),
        parse_mode="Markdown"
    )
...
def build_admin_conv():
    return ConversationHandler(
        entry_points=[
            ...
            # Books
            CallbackQueryHandler(admin_book_menu,               pattern="^admin_book_menu$"),
            ...
            CallbackQueryHandler(admin_book_del_confirm,        pattern=r"^abkdel_confirm_\d+$"),
            CallbackQueryHandler(admin_book_del_yes,            pattern=r"^abkdel_yes_\d+$"),
            # Others
        ...
        states={
            ...
            BOOK_FILE:          [MessageHandler(filters.Document.ALL, abk_file)],
            # Others
