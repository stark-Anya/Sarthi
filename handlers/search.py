...
async def search_cmd(update: Update, context):
    ...
    pyqs = conn.execute(
        """SELECT DISTINCT exam_type, title FROM pyqs
           WHERE LOWER(title) LIKE ? OR LOWER(exam_type) LIKE ?
           ORDER BY exam_type, title LIMIT 10""",
        (f"%{query_str}%", f"%{query_str}%")
    ).fetchall()

    mix_books = conn.execute(
        """SELECT DISTINCT book_name FROM mix_books
           WHERE LOWER(book_name) LIKE ?
           ORDER BY book_name LIMIT 10""",
        (f"%{query_str}%",)
    ).fetchall()

    conn.close()

    if not memories and not reports and not formulas and not books and not pyqs and not mix_books:
        await update.message.reply_text(
    ...
    if pyqs:
        ...
        rows = [
            [Btn(f"📄 {EXAM_LABELS.get(p['exam_type'], p['exam_type'])}: {p['title']}",
                 callback_data=f"pyq_open_{p['exam_type']}_{p['title']}")]
            for p in pyqs
        ]
        await update.message.reply_text(
            f"📋 *PYQs ({len(pyqs)}):*",
    ...
    if mix_books:
        rows = [
            [Btn(f"📗 {m['book_name']}", callback_data=f"mix_open_{m['book_name']}")]
            for m in mix_books
        ]
        await update.message.reply_text(
            f"🔀 *Mix Books ({len(mix_books)}):*",
            reply_markup=Markup(rows), parse_mode="Markdown"
        )
