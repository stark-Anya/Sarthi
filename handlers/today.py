...
from database import get_conn, get_user, update_streak, get_setting
...
async def score_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM test_scores WHERE user_id=? ORDER BY date DESC LIMIT 10", (uid,)
    ).fetchall()
    conn.close()
    ...
    if rows:
        kb_rows.append([Btn("🗑️ Delete a Score", callback_data="score_delete_pick")])
...
async def score_delete_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    uid = _uid(update)
    conn = get_conn()
    ...
    conn.close()
    if not rows:
        await query.edit_message_text("No scores to delete.", reply_markup=back_btn("score_home"))
        return ConversationHandler.END
    kb_rows = [
        [Btn(f"🗑️ {r['test_name']} ({r['date']})", callback_data=f"score_del_confirm_{r['id']}")]
        for r in rows
    ]
    kb_rows.append([Btn(f"{E['back']} Back", callback_data="score_home")])
    await query.edit_message_text("Select score to delete:", reply_markup=Markup(kb_rows))
    return ConversationHandler.END


async def score_del_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    score_id = int(query.data.split("_")[-1])
    await query.edit_message_text(
        "⚠️ *This cannot be undone.* Delete this test score?",
        reply_markup=confirm_delete_kb(f"score_del_yes_{score_id}", "score_home"),
        parse_mode="Markdown"
    )
    return ConversationHandler.END


async def score_del_yes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    score_id = int(query.data.split("_")[-1])
    conn = get_conn()
    conn.execute("DELETE FROM test_scores WHERE id=?", (score_id,))
    conn.commit()
    conn.close()
    await query.edit_message_text("🗑️ Score deleted.", reply_markup=back_btn("score_home"))
    return ConversationHandler.END
...
async def doubt_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ai_btn_label = "🤖 Ask AI Doubt" if api_key else "🤖 Ask AI (unavailable)"
    ...
    kb_rows.append([Btn("➕ Add Doubt",     callback_data="doubt_add"),
                    Btn(ai_btn_label,        callback_data="ai_doubt_start")])
    kb_rows.append([Btn(f"{E['back']} Back", callback_data="today_home")])
...
# ════════════════════════════════════════════════════════════════════════════
#  AI DOUBT — Gemini 2.5 Flash
# ════════════════════════════════════════════════════════════════════════════
async def ai_doubt_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    if not api_key:
        await query.edit_message_text(
            "🤖 *AI Doubt Solver*\n\n"
            "⚠️ *API not available.*\n\n"
            "Admin ne abhi Gemini API key set nahi ki hai.\n"
    ...
    await query.edit_message_text(
        "🤖 *AI Doubt Solver*\n\n"
        "Apna *question* type karo — Physics, Chemistry, Math, Biology kuch bhi!\n\n"
        "_(Jitna detail mein likhoge, utna accha answer milega)_",
        reply_markup=cancel_btn("doubt_home"), parse_mode="Markdown"
    )
    return AI_DOUBT_QUESTION


async def ai_doubt_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await check_banned(update):
        return ConversationHandler.END
    ...
    if not api_key:
        await update.message.reply_text(
            "⚠️ API key not available. Please contact admin.",
            reply_markup=back_btn("doubt_home")
        )
        return ConversationHandler.END

    # Show typing indicator
    thinking_msg = await update.message.reply_text(
        "🤖 _Thinking... please wait_",
        parse_mode="Markdown"
    )
    ...
    try:
        ...
        system_prompt = (
            "You are JEE Saarthi AI — a helpful study assistant for JEE and NEET aspirants. "
            "Answer questions clearly and concisely in the same language the student uses "
            "(Hindi/English/Hinglish). For science/math questions: show step-by-step solutions. "
        ...
        # Telegram markdown limit = 4096 chars
        if len(answer) > 3800:
            answer = answer[:3800] + "\n\n_...Answer truncated. Ask follow-up for more._"
        ...
        await update.message.reply_text(
            f"🤖 *AI Answer:*\n\n{answer}",
            reply_markup=Markup([
                [Btn("❓ Ask Another",    callback_data="ai_doubt_start"),
                 Btn("💾 Save as Doubt",  callback_data=f"ai_save_doubt")],
                [Btn(f"{E['back']} Back", callback_data="doubt_home")],
            ]),
            parse_mode="Markdown"
        )
        # Store last Q&A for save functionality
        context.user_data["ai_last_q"] = question
        context.user_data["ai_last_a"] = answer

    except urllib.error.HTTPError as e:
        await thinking_msg.delete()
        err_body = e.read().decode() if e.fp else ""
        if "API_KEY_INVALID" in err_body or e.code == 400:
            msg = "❌ Invalid API key. Admin should check the key."
        elif e.code == 429:
            msg = "❌ Rate limit exceeded. Please try again after a minute."
        else:
            msg = f"❌ API error ({e.code}). Please try again."
        await update.message.reply_text(msg, reply_markup=back_btn("doubt_home"))
...
async def ai_save_doubt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Save the AI Q&A as a doubt entry."""
    query = update.callback_query
    await query.answer()
    question = context.user_data.get("ai_last_q", "AI Doubt")
    uid  = _uid(update)
    conn = get_conn()
    conn.execute(
        "INSERT INTO doubts (user_id, subject, text) VALUES (?,?,?)",
        (uid, "AI", question[:500])
    )
    conn.commit(); conn.close()
    await query.answer("✅ Saved to your doubts!", show_alert=True)
    return ConversationHandler.END
...
def build_today_conv():
    return ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(score_delete_pick,  pattern="^score_delete_pick$"),
            CallbackQueryHandler(score_del_confirm,  pattern=r"^score_del_confirm_\d+$"),
            CallbackQueryHandler(score_del_yes,      pattern=r"^score_del_yes_\d+$"),
            ...
            CallbackQueryHandler(ai_doubt_start,    pattern="^ai_doubt_start$"),
            CallbackQueryHandler(ai_save_doubt,     pattern="^ai_save_doubt$"),
        ...
        states={
            ...
            AI_DOUBT_QUESTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, ai_doubt_answer)],
        },
        fallbacks=[
