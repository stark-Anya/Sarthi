...
async def job_doubt_reminder(app):
    """Remind users about doubts pending for 2+ days — runs at 8:00 PM IST."""
    from telegram import InlineKeyboardButton as Btn, InlineKeyboardMarkup as Markup
    from collections import defaultdict
    cutoff = (date.today() - timedelta(days=2)).isoformat()
    conn = get_conn()
    rows = conn.execute(
        """SELECT d.*, u.tg_id FROM doubts d
           JOIN users u ON d.user_id=u.id
           WHERE d.resolved=0 AND d.created<=? AND u.is_banned=0""",
        (cutoff + " 23:59:59",)
    ).fetchall()
    conn.close()
    user_doubts = defaultdict(list)
    for r in rows:
        user_doubts[r["tg_id"]].append(r)
    kb = Markup([[Btn("❓ Open Doubts", callback_data="doubt_home")]])
    for tg_id, doubts in user_doubts.items():
        lines = "\n".join([f"• [{d['subject'] or '?'}] {d['text'][:50]}" for d in doubts])
        text = (
            f"🔔 *Doubt Reminder!*\n\n"
            f"You have *{len(doubts)}* unresolved doubt(s) pending for 2+ days:\n\n"
            f"{lines}\n\n"
            f"_Don't let doubts pile up! Resolve them today._ 💪"
        )
        try:
            await app.bot.send_message(tg_id, text, reply_markup=kb, parse_mode="Markdown")
        except Exception as e:
            logger.error(f"Doubt reminder error for {tg_id}: {e}")


async def job_backup_db(app):
    """Send daily DB backup to BACKUP_GROUP_ID at 11:00 PM IST."""
    from config import BACKUP_GROUP_ID, DB_PATH
    try:
        ...
        logger.info(f"[Backup] DB backup sent to group {BACKUP_GROUP_ID}")
    except Exception as e:
...
def setup_scheduler(app):
    ...
    # 8:00 PM — doubt follow-up reminder
    scheduler.add_job(job_doubt_reminder, "cron", hour=20, minute=0, args=[app])
